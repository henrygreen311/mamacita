...
async function runBetCycle(page, isFirstRun = false) {      
  ...
  while (true) {      
    ...
    if (nextBtn) {      
      ...
      console.log('Next Round button not found. Skipping loop and refreshing page...');      
      break; // <-- skip this loop entirely      
    }      
  }      
...
(async () => {      
  ...
  // --- Loop for 5 hours ---      
  const runDuration = (5 * 60 + 50) * 60 * 1000; // 5 hours 50 minutes in milliseconds      
  ...
  async function main() {      
    ...
    console.log("5 hours elapsed. Closing browser.");      
    await browser.close();      
    process.exit(0);      
  }      
  ...
  setTimeout(async () => {      
    console.log("5 hours hard timeout reached. Closing browser.");      
    await browser.close();      
    process.exit(0);      
  }, runDuration);      
